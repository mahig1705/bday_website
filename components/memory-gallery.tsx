...
export default function MemoryGallery() {
  ...
  return (
    <div className="w-full h-full flex flex-col items-center p-4 overflow-y-auto">
      ...
      >
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-pink-600 mb-2 sm:mb-4">Our Special Moments</h2>
        <p className="text-base sm:text-lg text-purple-700">A collection of our favorite memories together</p>
      </motion.div>
      ...
      <div className="w-full max-w-6xl grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {memories.map((memory, index) => (
          ...
          >
            ...
            <div className="p-3">
              <h3 className="font-medium text-gray-800 text-sm sm:text-base">{memory.title}</h3>
            </div>
          </motion.div>
        ))}
      </div>
        ...
        {selectedMemory && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1 }}
              exit={{ scale: 0.8 }}
              className="relative w-full max-w-md sm:max-w-2xl bg-white rounded-lg overflow-hidden"
            ...
            >
              ...
              <div className="p-4 sm:p-6">
                <h3 className="text-lg sm:text-xl font-bold text-gray-800 mb-2">{selectedMemory.title}</h3>
                <p className="text-sm sm:text-base text-gray-600">{selectedMemory.caption}</p>
              </div>
            </motion.div>
          </motion.div>
        )}
